...
Runtime logs, screenshots, and overlays live under `ModsData/airimayor`. The product has not shipped, so unknown on-disk shapes are rejected and there is no migration; development data is moved by hand. Logs and screenshots stay until the player clears them.
