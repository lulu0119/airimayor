...
- Chat tool images: the preview keeps its ratio from the JPEG header in the page. Needs a live screenshot or map image that is not square.
...
- Plan: it survives autonomous continuation and player messages. The chat plan strip shows the steps after `set_plan`, and the empty state when there is no plan, not a transcript line.
...
- Service fee prices (`list_fees` / `set_budget` kind=fee): `sliderRange` matches the economy panel (minimum 0, maximum and ordinary price from the panel). On a new city, writing electricity's ordinary price succeeds and the panel shows that price.
- Local road topology: a too-close finding from `inspect_network_topology` carries `at` and two edges; a radius that misses those junctions does not repeat them.
