...
- Tool image preview (thumbnail data URI plus source pixel size on the tool done event, rendered in the tool row): needs in-game acceptance — screenshot/`map_image` rows show the image at its source aspect, text-only rows unchanged, session switch still hydrates.
...
- Renamed tool surface ([0027](./adr/0027-tool-surface-renaming.md): `zone`, `build_network`, `set_budget`, `get_`/`list_` reads, `add/remove_transit_line`): needs a live run proving zoning, a network build, a finance write and a transit add/remove all dispatch on the new names with no old-name residue in prompt or catalog.
