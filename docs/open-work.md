...
- `map_image` (native geometry primary with Carto fallback, vision-gated): needs Windows `dotnet build` plus a live run with no Carto install proving the PNG attaches in seconds, overpasses stack, transit loops show, chat preview shows, and no 400 pairing errors in following turns (needs a fresh session; old histories carry the pre-fix interleaved image).
- Native `map_image` frames are always game meters (bounded clips directly, water always samples; Game CRS forcing and the meter heuristic now cover only the Carto fallback). Needs a live run proving a bounded call returns a zoomed PNG instead of Unavailable.
