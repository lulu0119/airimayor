...
- After wait_simulation, use the nested overview and problems as the city snapshot: notificationCounts are citywide icon totals; serviceGaps are sewage/water/electricity capacity gaps (id/severity/message). Call notifications only when you need raw icon locations or targets.
...
- Expand roads outward on owned tiles in short segments; keep utilities ahead of demand and give each new road a network role before zoning it. Use local_map at the work site before choosing a connection or expansion. Prefer closing loops and meshes in zoned areas rather than leaving degree-1 streets as the only access. Keep junctions at least 32 m apart.
- wait_simulation(hours) advances the requested 1-24 in-game hours (default 1) at the engine 8x speed cap and then restores the previous speed/pause state. The success payload nests overview (population, money, xp, ...) and problems (notificationCounts, serviceGaps) with hours/completed/targetReached/note -- use that as the first snapshot and after growth batches. Buildings take game hours to construct, level up and attract residents: use 1-2 hours to verify a repair or capacity change, and about 4 hours after a normal growth batch. When problems show no notificationCounts and no serviceGaps, a positive budget and ample utility headroom, use an 8-12 hour growth window before waiting again and re-reading budget.
...
- When wait_simulation problems.notificationCounts include Traffic Bottleneck Notification, or list_networks shows congestion, diagnose first: use local_map at the congested site; inspect_network_topology(kind=road) for near_miss (a degree-1 dead end within 32 m of another road that does not share a node), unnoded_crossing, too_close_junctions (degree >= 3 nodes closer than 32 m), and isolated_road; and list_networks(kind=road, sort=congestion or traffic_volume). Degree-1 dead ends farther than 32 m are facts, not automatic errors. Isolation for pipes/cables uses inspect_network_topology(kind=water|sewage|low_voltage), not list_networks.
...
- After a batch of road writes, inspect_network_topology(kind=road). After the write, wait_simulation (1-2 hours for a local repair) and re-measure the same sorted list_networks plus the wait digest. If congestion is unchanged, change the hierarchy rather than repeating the same local write.
...
- Empty land: connect a road from the highway, then choose unlocked power, water and sewage prefabs with list_prefabs and place them with x/z/radius. Non-wind power needs a transformer and a hand-built high-voltage cable; read the utility-networks section. Use local_map at the site before choosing an expansion direction or when shoreline/slope evidence matters; its LOCAL_MAP frame, regions, sectors and road topology are compact spatial evidence, while the write tools remain responsible for native validation.
...
- Sewage is critical: if serviceGaps show sewage or notificationCounts include Sewage Notification, BUILD SewageOutlet01 near water immediately. place_building SewageOutlet01 with x/z + radius snaps to a legal shoreline. After building, wait_simulation() once and re-check serviceGaps. Population cannot grow with sewage backing up.
...
- Connect existing passenger stops into a line. Vehicles spawn from depots on their own. Stops already exist as station sub-stops (place a Bus Station with place_building) or roadside stop objects. There is no place_building role that creates a stop.
...
- Do not invent a stop by calling place_building with a transport role. Do not treat taxi stands, cargo lines, or work routes as this passenger-line slice. Do not delete a station building to remove a line.
