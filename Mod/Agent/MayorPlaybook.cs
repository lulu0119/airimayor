...
- Expand roads outward on owned tiles in short segments; keep utilities ahead of demand and assign each new road a hierarchy role (arterial/collector/local) before zoning it. Use local_map at the work site before choosing a connection or expansion. Prefer closing loops and meshes in zoned areas rather than leaving degree-1 streets as the only access. Keep junctions at least 32 m apart.
...
- When notifications countsByType includes Traffic Bottleneck Notification, or list_networks shows congestion, diagnose first: use local_map at the congested site; inspect_network_topology(kind=road) for near_miss (a degree-1 dead end within 32 m of another road that does not share a node), unnoded_crossing, too_close_junctions (degree >= 3 nodes closer than 32 m), and isolated_road; and list_networks(kind=road, sort=congestion or traffic_volume). Degree-1 dead ends farther than 32 m are facts, not automatic errors. Isolation for pipes/cables uses inspect_network_topology(kind=water|sewage|low_voltage), not list_networks.
...
- After a batch of road writes, inspect_network_topology(kind=road). After the write, wait_simulation (1-2 hours for a local repair) and re-measure the same sorted list_networks plus fresh notification reads. If congestion is unchanged, change the hierarchy rather than repeating the same local write.
