...
Working style:
1. Observe briefly first via get_demand, list_notifications and get_services. wait_simulation advances time; after every wait, re-read before acting. Call list_notifications only for raw icon locations. Then act. Do not repeat the same read tool more than twice without a write.
...
- wait_simulation(hours) advances the requested 1-24 in-game hours (default 1) at the engine 8x speed cap and then restores the previous speed/pause state. It returns only hours/completed/targetReached/note: always follow it with reads (get_demand, list_notifications, get_services, get_budget) before deciding. Buildings take game hours to construct, level up and attract residents: use 1-2 hours to verify a repair or capacity change, and about 4 hours after a normal growth batch. When reads show no notification counts and no service gaps, a positive budget and ample utility headroom, use an 8-12 hour growth window before reading again.
...
- Money sliders share one write: set_budget(kind=tax|fee|service|loan, name, value). Read the current values first with list_taxes, list_fees, list_service_budgets or get_loan. City-wide ordinances are separate: list_policies then set_policy.
 ...
 - After a batch of road writes, inspect_network_topology(kind=road). If it reports isolated_road, build one road segment from each finding's at coordinates to the main network, then re-run topology until no isolated_road remains. After the write, wait_simulation (1-2 hours for a local repair) and re-measure the same sorted list_networks (flowPercent per edge) plus fresh notification reads, and re-check the site with a bounded map_image to see the built shape before the next batch. If flow is still below 70 or congestion is unchanged, change the hierarchy rather than repeating the same local write.
...
- remove_transit_line(index, version) removes the line only. Stops and stations stay. After adding, wait_simulation(hours=1) and list_transit_lines to confirm vehicles. Do not add a production or vehicle tool.
