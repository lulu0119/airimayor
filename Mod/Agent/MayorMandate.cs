...
namespace CitiesSkylines2Agent.Agent
{
    ...
    internal sealed class MayorMandate
    {
        ...
        private static readonly JsonDocument ParametersDocument = JsonDocument.Parse(
            @"{
                ...
                ""properties"": {
                    ...
                    ""success"": {
                        ""type"": ""string"",
                        ""description"": ""Measurable stop condition, such as Traffic Bottleneck count or flowPercent 70""
                    }
                },
        ...
        public MandateCallResult SetPlan(string argumentsJson)
        {
            if (!TryReadField(argumentsJson, "goal", out string goal))
            {
                return MandateCallResult.Fail("goal is required");
            }
            if (!TryReadField(argumentsJson, "success", out string success))
            {
                return MandateCallResult.Fail("success is required");
            }
            ...
            lock (m_Gate)
            {
                m_Goal = goal;
                m_Success = success;
                return MandateCallResult.Ok(ToUiJsonUnlocked());
            }
        }
            ...
            lock (m_Gate)
            {
                if (string.IsNullOrEmpty(m_Goal))
                {
                    return HistoryNotePrefix +
                        "none. Call set_plan when you can name one city goal.";
                }

                return HistoryNotePrefix + "goal=" + m_Goal +
                    " success=" + m_Success +
                    ". Continue this plan. Do not start a citywide review or a new growth push until this plan is met or you call set_plan to replace it.";
            }
        }
    ...
    internal readonly struct MandateCallResult
    {
        public readonly bool Success;
        public readonly string Text;

        private MandateCallResult(bool success, string text)
        {
            Success = success;
            Text = text;
        }

        public static MandateCallResult Ok(string text)
        {
            return new MandateCallResult(true, text);
        }

        public static MandateCallResult Fail(string message)
        {
            return new MandateCallResult(
                false,
